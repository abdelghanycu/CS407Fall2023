...
public class ASTGenerator {
  ...
  private static String readFile() throws IOException {
    File file = new File("java_example.java");
  ...
  private static void generateAST(RuleContext ctx, boolean verbose, int indentation) {
    boolean toBeIgnored =
        !verbose && ctx.getChildCount() == 1 && ctx.getChild(0) instanceof ParserRuleContext;
  ...
  private static void printDOT() {
    ...
    for (int i = 1; i < LineNum.size(); i++) {
      ...
      System.out.println(
          (Integer.parseInt(LineNum.get(i)) - 1) + Integer.toString(pos) + "->" + LineNum.get(i)
              + i);
    }
  }
  ...
  private static void printLabel() {
    for (int i = 0; i < LineNum.size(); i++) {
      System.out.println(
          LineNum.get(i) + i + "[label=\"" + Type.get(i) + "\\n " + Content.get(i) + " \"]");
    }
  }
